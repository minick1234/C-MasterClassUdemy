...
namespace UDEMYMASTERCLASSFORCSHARP
{
    public class StackAndQueuesLesson
    {
        public static void main(string[] args)
        {
            ...
            if (stackOfInts.Count > 0) stackOfInts.Pop();
            ...
            {
                var tempInt = stackOfInts.Pop();
                Console.WriteLine("The current stacks popped int is: {0}", tempInt);
                Console.WriteLine("The remaining count of stack items is: {0}\n", stackOfInts.Count);
            }

            if (stackOfInts.Count == 0) Console.WriteLine("The stack is out of stuff to pop and is now empty.");
            ...
            for (var i = 0; i < counter; i++) reversedArray[i] = reverseNums.Pop();

            Console.WriteLine("The nums in reverse order are: ");
            foreach (var num in reversedArray) Console.WriteLine(num + "\t");
        }
    }
}